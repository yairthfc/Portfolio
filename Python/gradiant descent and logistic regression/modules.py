...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        output = (self.weights_ ** 2).sum(axis=-1)
        return output
...
class L1(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        output = np.abs(self.weights_).sum(axis=-1)
        return output
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        jacobian = np.sign(self.weights_)
        return jacobian
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        log_likelihood = - (1 / m) * np.sum(y * z - np.log(1 + np.exp(z)))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        z = np.dot(X, self.weights)
        ez = np.exp(z)
        gradient = - np.sum((y - (ez / (1 + ez)))[:, None] * X, axis=0) / m
        return gradient
...
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        return np.concatenate(([fidelity_jacobian[0]], fidelity_jacobian[1:] + self.lam_ * regularization_jacobian))
