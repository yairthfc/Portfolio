...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        iteration, change, previous_weights = 0, self.tol_, np.copy(f.weights)
        best_value, best_weights, cumulative_weights = np.Inf, None, np.zeros_like(f.weights)

        while iteration < self.max_iter_ and change >= self.tol_:
            value = f.compute_output(X=X, y=y)
            gradient = f.compute_jacobian(X=X, y=y)
            learning_rate = self.learning_rate_.lr_step(f=f, x=X, dx=-gradient, t=iteration)
            f.weights -= learning_rate * gradient
            cumulative_weights += f.weights

            change = np.linalg.norm(f.weights - previous_weights)
            previous_weights = np.copy(f.weights)
            iteration += 1

            if value < best_value:
                best_value, best_weights = value, np.copy(f.weights)

            self.callback_(solver=self, weight=np.copy(f.weights), val=value, grad=gradient, 
                        iteration=iteration, learning_rate=learning_rate, change=change)

        if self.out_type_ == "last":
            return f.weights
        if self.out_type_ == "best":
            return best_weights
        return cumulative_weights / iteration
