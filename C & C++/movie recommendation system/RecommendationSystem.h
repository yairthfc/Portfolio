...
typedef std::map<sp_movie, features, CompSpMovie> ranks_map;


class RecommendationSystem
{
    ranks_map* _ranks_map;
public:
    RecommendationSystem(){_ranks_map = new ranks_map;}
    ...
    double scalar_prod (const std::vector<double> &v1,
									  const std::vector<double> &v2);

    double get_degree (const std::vector<double> &v1, const
    std::vector<double> &v2);
    double vec_size (const std::vector<double> &vec);
