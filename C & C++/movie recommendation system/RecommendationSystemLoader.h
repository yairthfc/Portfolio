...
class RecommendationSystemLoader {
 ...
 public:
  ...
  /**
   * loads movies by the given format for movies
   * with their feature's score
   * @param movies_file_path a path to the file of the movies
   * @return smart pointer to a RecommendationSys
   * tem which was created with
   * those movies
   */
  static std::unique_ptr<RecommendationSystem>
  create_rs_from_movies
